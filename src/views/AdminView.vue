...
<script lang="js" setup>
	...
	const totalAcessosÚnicos = ref(0);
	const totalAcessosPeríodo = ref(0);
	const totalAcessosPeríodoÚnicos = ref(0);
	const dates = ref([]);
	...
	onMounted(async () => {
		...
		totalAcessosÚnicos.value = obterObjetosÚnicosPorIPUnidade(unidades.value);
		...
		watch(dates, () => {
			if (dates.value) {
				...
				if (startDate && endDate) {
					...
					totalAcessosPeríodo.value = contarTotalAcessos(unidades.value);
					totalAcessosPeríodoÚnicos.value = obterObjetosÚnicosPorIPUnidade(unidades.value);
				}
			} else {
				dates.value = [];
				totalAcessosPeríodo.value = 0;
				totalAcessosPeríodoÚnicos.value = 0;
