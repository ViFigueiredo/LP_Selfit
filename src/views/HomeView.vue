<template>
  <div class="space-y-12">
    <div id="modal-shadow" class="z-10 absolute top-0 w-full h-screen bg-red-300" v-if="showModal" @click="toggleModal">
      <div id="modal" class="bg-black overflow-y-auto p-6 w-full h-full rounded-xl flex flex-col space-y-5" @click.stop>
            ...
            <div v-for="(unidades, cidade) in cidades" :key="cidade">
              ...
              <ul v-if="showUnidades[uf] && showUnidades[uf][cidade]">
                <li v-for="unidade in unidades" :key="unidade.cidade"
                  ...
                  <a :href="unidade.url" target="_blank"
    ...
    <div class="floater-whatsapp">
      <a href="https://wa.link/uq6pis">
        <i>
          <svg class="z-10 w-[60px] h-[60px] bg-[#25D366] rounded-full bg-green fixed bottom-5 right-5"
    ...
    <footer class="relative bottom-0 w-full flex justify-center items-center text-center p-2 bg-red-600 text-white">
      <span class="w-full">
        ...
        Soluções.
        <span @click="toPanel()" class="text-red-600 justify-start hover:text-white hover:cursor-pointer">
          Admin
        </span>
      </span>
    </footer>
