<template>
  <div class="space-y-12">
    ...
    >
      <div
        id="modal"
        class="bg-black overflow-y-auto p-6 w-full h-full rounded-xl flex flex-col space-y-5"
        @click.stop
      >
        ...
        <div v-for="(cidades, uf) in agrupadosPorUF" :key="uf">
          ...
          <div
            v-if="showCidades[uf]"
            class="border border-2 border-solid border-red-600 p-5"
          >
              ...
              <ul v-if="showUnidades[uf] && showUnidades[uf][cidade]">
                ...
                >
                  <a
                    :href="unidade.url"
                    target="_blank"
                    class="flex justify-between w-full cursor-pointer font-bold pt-3 space-x-3"
    ...
    <div class="flex flex-col">
      ...
      >
        <img
          class="w-full h-60 lg:h-[350px]"
          src="@/assets/img/Banner-2025-verao.png"
          alt="plano plus"
        />
        <div
    ...
    <div
      class="w-full flex flex-col justify-center items-center px-5 space-y-5 md:space-y-0 md:space-x-12 md:flex-row"
    >
      <div id="carousel" class="relative h-80 w-[80%] md:h-50 md:w-[50%]">
        ...
        >
          ...
          <div
            id="text-carousel"
            class="absolute w-full bottom-0 p-7 text-white"
          >
    ...
    <div class="floater-whatsapp">
      <a href="https://wa.link/uq6pis">
        <i>
          <svg
            class="z-10 w-[60px] h-[60px] bg-[#25D366] rounded-full bg-green fixed bottom-5 right-5"
            viewBox="0 0 24 24"
            fill="none"
    ...
    >
      <form
        class="flex flex-col space-y-3 border-2 border-red-600 p-3"
        @submit.prevent="adminPanel()"
      >
        <div class="space-x-3">
        ...
        <div class="w-full flex justify-end items-center">
          <button class="bg-green-600 rounded-md text-white p-2" type="submit">
            Entrar
          </button>
        </div>
      </form>
    ...
    <footer
      class="relative bottom-0 w-full flex justify-center items-center text-center p-2 bg-red-600 text-white"
    >
      <span class="w-full">
        2024 Selfit © Todos os direitos reservados. Desenvolvido por Center
        Soluções.
        <span
          @click="toPanel()"
          class="text-red-600 justify-start hover:text-white hover:cursor-pointer"
        >
          Admin
        </span>
      </span>
...
const agrupadosPorUF = computed(() => {
  ...
  for (let uf in grupos) {
    grupos[uf] = Object.keys(grupos[uf])
      .sort()
      .reduce((obj, key) => {
        obj[key] = grupos[uf][key].sort((a, b) =>
...
function adminPanel() {
  if (
    username.value === "CenterSelfit@#2024!" &&
    password.value === "CenterSelfit@#2024!"
  ) {
